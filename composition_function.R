...
Raster_composition<-function(out_dir,input_dir,pattern_r,filename){
...
writeRaster(final4,paste0(out_dir,"/",filename,"_RAT.tif"), RAT=TRUE)           ##SAVING GeoTIFF Results with raster attribute table
writeRaster(final3,paste0(out_dir,"/",filename,".tif"))                         ##SAVING GeoTIFF Results
write.csv(val,paste0(out_dir,"/",filename,"_IDs.csv"),quote = F,row.names = F)  ##SAVING IDs IN A CSV FILE
...
###INPUT PATH WHERE THE RASTERS FILES ARE###
input_dir<-""
###PUT CURRENT AND PROJECTED SDM FOR A RCP 

pattern_r<-"" #RASTER FILES NAME

###INPUT PATH WHERE THE OUTCOMES FILES WILL SAVED###
out_dir<-""
filename<-"" #CURRENT FILE OUTCOME NAME


###RUNNING FUNCTION###

x<-Raster_composition(out_dir=out_dir,input_dir=input_dir,pattern_r=pattern_r,filename=filename)
